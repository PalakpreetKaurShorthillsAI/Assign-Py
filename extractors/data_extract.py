...
class DataExtractor:
    ...
    def extract_text(self):
        ...
        #     reader = self.loader.load_file()  # Should return the PdfReader object
        #     text = ""
        #     for page_num in range(reader.page_count):  # Iterate using page count
        #         page = reader.load_page(page_num)  # Load each page individually
        #         text += page.get_text("text")  # Use get_text() with format "text"
        #     return text
        if self.file_path.endswith('.pdf'):
        # Extract text from PDF using PyMuPDF
            reader = fitz.open(self.file_path)  # Open the PDF with PyMuPDF
            text = ""
            for page_num in range(reader.page_count):  # Iterate using page count
                page = reader.load_page(page_num)  # Load each page individually
                text += page.get_text("text")  # Use get_text() with format "text"
    ...
    def extract_links(self):
        ...
        if self.file_path.endswith('.pdf'):
            ...
            for rel in doc.part.rels.values():
                if rel.target_ref.startswith("http"):  # Check if the URL is valid
                    urls.append({
                        "url": rel.target_ref,
                        "page": None,  # DOCX does not have a page number context
                        "position": None  # No positional information in DOCX
                    })
            ...
            for slide in ppt.slides:
                for shape in slide.shapes:
                    if hasattr(shape, "table"):
                        table = shape.table
                        table_content = [[cell.text for cell in row.cells] for cell in table.rows]
                        table_data.append(table_content)
            return table_data
